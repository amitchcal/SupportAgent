...
export async function POST(request: Request) {
  ...
  if (body.action === "start") {
    ...
    const classification = applyIndustryClassification(classifyIssue(message), message, tenant.settings); const safetyReasons = detectSafetyRisk(message); const issue = extractIssueDetails(message, contact);
