...
export async function POST(request: Request) {
  ...
  if (body.action === "start") {
    ...
    return Response.json({ conversationId, reply: result.content, status: result.status, classification, issue, ticketReference: ticket?.reference });
  }
  ...
  if (body.action === "start_sop") {
    ...
    return Response.json({ conversationId, reply, status: execution.status === "ESCALATED" ? "ESCALATED" : "SOP_IN_PROGRESS", sop: { id: sop.id, title: sop.title, version: sop.version }, step, ticketReference: ticket?.reference });
  }
  ...
  if (body.action === "sop_step") {
    ...
    return Response.json({ conversationId, reply, status, step: nextStep, ticketReference: ticket?.reference });
  }
