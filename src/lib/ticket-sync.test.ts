...
const ticket: Ticket = { id: "ticket-1", reference: "SUP-1", tenantId: "tenant-a", conversationId: "conversation-1", integrationId: "integration-1", status: "CREATED", externalTicketId: "INC001", externalTicketUrl: null, normalizedPayload: { schemaVersion: "1.0", tenantId: "tenant-a", conversationId: "conversation-1", requester: {}, asset: { id: "", model: "", serialNumber: "", site: "" }, issue: { summary: "Motor alarm", category: "Electrical", severity: "medium", urgency: "soon", errorCode: "", safetyReasons: [] }, troubleshooting: [], attachments: [], transcript: [] }, creationAttempts: 1, lastError: null, createdAt: now, updatedAt: now };
...
test("inbound status updates are tenant scoped, idempotent and logged", () => {
  ...
  applyExternalStatus(db, "tenant-a", { externalTicketId: "INC001", status: "Resolved", raw: {} }, "STATUS_WEBHOOK", now); assert.equal(db.knowledgeCandidates?.length, 1);
});
