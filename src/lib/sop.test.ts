...
test("only active SOPs from the same tenant can be selected or executed", () => {
  const database = { tenants: [], users: [], auditLogs: [], conversations: [], conversationMessages: [], knowledgeDocuments: [], knowledgeVersions: [], sopDefinitions: [sop, { ...sop, id: "draft", status: "DRAFT" as const }], sopExecutions: [] } satisfies Database;
