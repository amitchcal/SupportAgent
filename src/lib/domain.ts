...
export type Conversation = { id: string; tenantId: string; language: SupportedLanguage; channel?: "CHAT" | "VOICE" | "WHATSAPP"; contact: Record<string, string>; issue: IssueDetails; classification: Classification; safetyReasons: string[]; lowConfidenceReason: string | null; clarificationCount: number; status: ConversationStatus; createdAt: string; updatedAt: string; closedAt?: string | null };
...
export type KnowledgeCandidate = { id: string; tenantId: string; sourceTicketId: string; sourceTicketReference: string; title: string; proposedContent: string; category: IssueCategory; status: "PENDING" | "CONVERTED" | "DISMISSED"; reviewedBy: string | null; linkedDocumentId: string | null; createdAt: string; updatedAt: string };

export type RateLimitEntry = { key: string; count: number; windowStartedAt: string };
export type MessagingReceipt = { id: string; tenantId: string; provider: "WHATSAPP"; externalMessageId: string; senderId: string; conversationId?: string | null; status: "PROCESSING" | "PROCESSED" | "FAILED"; error: string | null; createdAt: string; updatedAt: string };
export type Database = { tenants: Tenant[]; users: User[]; auditLogs: AuditLog[]; conversations: Conversation[]; conversationMessages: ConversationMessage[]; conversationFeedback: ConversationFeedback[]; knowledgeDocuments: KnowledgeDocument[]; knowledgeVersions: KnowledgeVersion[]; knowledgeGaps?: KnowledgeGap[]; knowledgeCandidates?: KnowledgeCandidate[]; industryTemplates?: IndustryTemplate[]; rateLimits?: RateLimitEntry[]; messagingReceipts?: MessagingReceipt[]; sopDefinitions: SopDefinition[]; sopExecutions: SopExecution[]; ticketingIntegrations: TicketingIntegration[]; tickets: Ticket[]; ticketSyncLogs: TicketSyncLog[] };
