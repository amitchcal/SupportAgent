...
export type Conversation = { id: string; tenantId: string; language: SupportedLanguage; contact: Record<string, string>; issue: IssueDetails; classification: Classification; safetyReasons: string[]; lowConfidenceReason: string | null; clarificationCount: number; status: ConversationStatus; createdAt: string; updatedAt: string; closedAt?: string | null };
...
export type Database = { tenants: Tenant[]; users: User[]; auditLogs: AuditLog[]; conversations: Conversation[]; conversationMessages: ConversationMessage[]; conversationFeedback: ConversationFeedback[]; knowledgeDocuments: KnowledgeDocument[]; knowledgeVersions: KnowledgeVersion[]; sopDefinitions: SopDefinition[]; sopExecutions: SopExecution[]; ticketingIntegrations: TicketingIntegration[]; tickets: Ticket[]; ticketSyncLogs: TicketSyncLog[] };
